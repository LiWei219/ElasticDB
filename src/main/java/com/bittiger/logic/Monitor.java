...
public class Monitor {
	...
	private void updateStats(double x, double u, double r, double w, double m, double rq, double wq) {
		try {
			Statement stmt = con.createStatement();
			StatsQuery stats = new StatsQuery(x, u, r, w, m, rq,wq);
			stmt.executeUpdate(stats.getQueryStr());
			stmt.close();
			LOG.info("Stats: Interval:" + x + ", Queries:" + u + ", Read:" + r + ", Write:" + w + ", Nodes:" 
			+ m + ",Readqueries:" + rq + ",WriteQueries:" + wq);
		} catch (Exception e) {
			LOG.error(e.toString());
		}
	}
	...
	public String readPerformance() {
		...
		updateStats(x, totCount, avgRead, avgWrite,
				c.getLoadBalancer().getReadQueue().size(),readquery,writequery);
